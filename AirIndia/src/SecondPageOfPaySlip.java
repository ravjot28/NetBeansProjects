...
public class SecondPageOfPaySlip
{
    ...
    public int tot;
    String rav;
    public void launch(String[] data,String time) throws DocumentException, FileNotFoundException, BadElementException,IOException
    {
        String al[]=null;
        String de[]=null;
        getinfo(data[0]);
 ...
 else
     {
        ...
        PdfWriter.getInstance(document,new FileOutputStream(new Dir().getdir()+"test/Temp2/Temp3/Temp4/Temp5/Temp6/Temp7/Temp8/Temp9/Temp10/Temp11/Temp12/Pli_Slips.pdf"));
        document.open();
        ...
        document.add(new Paragraph("            EMP NO : "+empid+"                                                            INC DT & CD: "+inc+"/"+inccode,f1));
        document.add(new Paragraph("            EMP NAME : "+name+"                                     PAY SCALE : 16400-20300",f1));
        document.add(new Paragraph("            DESIGNATION: "+new desg().getdesg(desigcode)+"                              DESG CODE: "+desigcode,f1));
        document.add(new Paragraph("            CESS CODE : "+cesscode+"                                                                BANK CODE : "+bankcode,f1));
        document.add(new Paragraph("            SEX : "+sex+"                                                                    A/C NUMBER "+accno,f1));
...
if((al.length==1)&&(al[0].equalsIgnoreCase("0")))
      ...
      document.add(table);
        ...
        else
        {
            ...
            String[] headers = new String[] {"EMOLUMENTS","DEUCTIONS"};
      PdfPTable table = new PdfPTable(headers.length);
            for (int i = 0; i < headers.length; i++) {
                String header = headers[i];
                PdfPCell cell = new PdfPCell();
