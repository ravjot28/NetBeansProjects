...
import com.ravsofts.process.ModifyEmpID;
import com.ravsofts.sendreceievedata.SendEmpID;
 ...
 * @author Ravjot
 */
public class MembershipFormAgreementController implements Initializable, ControlledScreen {
    ...
    public void agreeHandler() {
        ...
        List<NomineeDetails> nominees = ((MembershipFormNomineesDetailsController) myController.getControlledScreen(GlobalConstants.nomineeDetailsScreen)).nominees;
        EmployeeDetails employeeDetails = ((MembershipFormEmployeeDetailsController) myController.getControlledScreen(GlobalConstants.employeeDetailsScreen)).employeeDetails;
        ...
        ContactInfo contactInfo = ((MembershipFormContactDetailsController) myController.getControlledScreen(GlobalConstants.contactDetailsScreen)).contactInfoDetails;
                ...
                public Void call() {
                    try {
                        ...
                        for (NomineeDetails nd : nomineeDetails) {
                            ...
                            if (nd.getRelative().equalsIgnoreCase("w/o")) {
                                relative = "W/o";
                            } else if (nd.getRelative().equalsIgnoreCase("s/o")) {
                                relative = "S/o";
                            } else if (nd.getRelative().equalsIgnoreCase("d/o")) {
                                relative = "D/o";
                            }
                            nomineeDetail += nd.getFullName() + "||" + relative + "||"
                                    + nd.getRelativeInfo() + "||" + nd.getAge() + "||" + nd.getProofs() + "||"
                        ...
                        if (ser.process()) {
                            ...
                            if (d == null) {
                                se = new SendEmpID("1", requestHeader + ",");
                            } else {
                                se = new SendEmpID("1", d[0].trim() + requestHeader + ",");
                            }
                            if (se.process()) {
                                validation = true;
                            } else {
    ...
    public void passedValidation() {
        Platform.runLater(new Runnable() {
            public void run() {
                Dialogs.showInformationDialog(null, null, GlobalConstants.internetConnectionErrorMessage, GlobalConstants.ERROR);
