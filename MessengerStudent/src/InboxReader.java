...
public class InboxReader
{
   ...
   static String yr;
   static String bran;
	InboxReader(String a,String bi)
        {
            yr=a;
            bran=bi;
				...
				for(Message message:messages)
                                ...
                                if(p.length()!=0)
                                {
                                    ...
                                    for(int i2=0;i2<aaa.length;i2++)
                                    {
                                        ...
                                        if((year.contains(yr))&&(branch.contains(bran)))
                                        {
...
//                                    new TestSlideInNotification(aaa);
           ...
           if(exception)
           {
               try{ store.close();BufferedWriter b=new BufferedWriter(new FileWriter("id.ravs")); b.append(messagess); b.close();new InboxReader(yr,bran);}catch(Exception asd){}
           }
 else
           {
                                    startreading();
