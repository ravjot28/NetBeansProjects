...
public class MediaControl extends BorderPane {
    ...
    public MediaControl(final MediaPlayer mp) {
        ...
        mvPane.setStyle("-fx-background-color: cornsilk;");
        setCenter(mvPane);
        ...
        final Button recordButton = new Button(BLACK_BOX);
        ...
        recordButton.setOnAction(new EventHandler<ActionEvent>() {
            public void handle(ActionEvent e) {
                ...
                if(!RECORDING.equals(recordingStatus)){
                    recordButton.setStyle("-fx-background-color: red;");
                    recordTask = new RecordTask();
                    try {
                        recordingStatus = recordTask.call();
                    } catch (Exception ex) {
                        Logger.getLogger(MediaControl.class.getName()).log(Level.SEVERE, null, ex);
                    }
                    Thread th = new Thread(recordTask);
                    threadList.add(th);
                    th.start();
                }else{
                    if(0 != threadList.size()){
                        recordButton.setStyle("");
                        SaveRecordTask saveRecordTask = null;
                        saveRecordTask = new SaveRecordTask();
                        try {
                            recordingStatus = saveRecordTask.call();
                        } catch (Exception ex) {
                            Logger.getLogger(MediaControl.class.getName()).log(Level.SEVERE, null, ex);
                        }
        ...
        mp.setOnEndOfMedia(new Runnable() {
            public void run() {
                if (!repeat) {
                    ...
                    atEndOfMedia = true;
                    mp.stop();
                    timeSlider.setValue(mp.getCurrentTime().toMillis()*100.0);
                }
            }
        });

        mediaBar.getChildren().add(playButton);
        Label shortSpacer = new Label(" ");
        mediaBar.getChildren().add(shortSpacer);
